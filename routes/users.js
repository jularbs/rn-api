...
router.get('/', authenticate, authorize('admin'), async (req, res) => {
  try {
    ...
    // Build query object
    const query = { isDeleted: { $ne: true } }; // Explicitly exclude soft deleted users
    ...
    const users = await User.find(query)
      ...
      .populate('preferences.favoriteStations', 'name frequency location');
...
router.delete('/:id', authenticate, authorize('admin'), async (req, res) => {
  try {
    const { id } = req.params;
    const deletedBy = req.user._id; // Get the admin user who is performing the deletion
    ...
    const stats = await User.findWithDeleted().aggregate([
      {
        $group: {
          ...
          allUsers: { $sum: 1 },
          activeUsers: {
            $sum: { $cond: [{ $and: [{ $eq: ['$isActive', true] }, { $ne: ['$isDeleted', true] }] }, 1, 0] }
          },
          adminUsers: {
            $sum: { $cond: [{ $and: [{ $eq: ['$role', 'admin'] }, { $ne: ['$isDeleted', true] }] }, 1, 0] }
          },
          verifiedUsers: {
            $sum: { $cond: [{ $and: [{ $eq: ['$emailVerified', true] }, { $ne: ['$isDeleted', true] }] }, 1, 0] }
          }
        }
      }
    ]);

    const recentUsers = await User.find({ isActive: true, isDeleted: { $ne: true } })
...
// GET /api/users/with-deleted - Get all users including deleted ones (Admin only)
router.get('/with-deleted', authenticate, authorize('admin'), async (req, res) => {
  try {
    const {
      page = 1,
      limit = 10,
    ...
    // Execute query with pagination (including deleted users)
    const users = await User.findWithDeleted()
      .find(query)
      .select('-password -emailVerificationToken -passwordResetToken -passwordResetExpires')
      .sort(sortOptions)
    ...
    res.status(500).json({
      success: false,
      message: 'Error fetching users with deleted',
      error: error.message
    });
  }
});
