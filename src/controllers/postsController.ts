...
export const updatePost = async (req: Request<{ id: string }, {}, UpdatePostRequest>, res: Response): Promise<void> => {
  try {
    ...
    if (updateData.category !== undefined)
      update.category = updateData.category ? new Types.ObjectId(updateData.category) : null;
    ...
    if (updateData.featuredImage !== undefined)
      update.featuredImage = updateData.featuredImage ? new Types.ObjectId(updateData.featuredImage) : null;
    if (updateData.thumbnailImage !== undefined)
      update.thumbnailImage = updateData.thumbnailImage ? new Types.ObjectId(updateData.thumbnailImage) : null;
    if (updateData.status !== undefined) update.status = updateData.status;
    if (updateData.scheduledAt !== undefined)
      update.scheduledAt = updateData.scheduledAt ? new Date(updateData.scheduledAt) : null;
